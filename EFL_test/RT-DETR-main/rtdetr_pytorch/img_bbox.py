...
def resize_with_padding(image, target_size):
    """
    이미지의 비율을 유지하면서 target_size로 리사이즈하고, 남는 공간에 패딩을 추가하는 함수
    """
    # 원본 이미지 비율을 유지하면서 target_size에 맞게 리사이즈
    image.thumbnail(target_size, Image.Resampling.LANCZOS)
    
    # 패딩을 추가하여 target_size에 맞춥니다
...
# ONNX 모델을 로드
ort_session = ort.InferenceSession('/EFL_test/RT-DETR-main/rtdetr_pytorch/BCE_model.onnx')

# # 이미지 전처리를 위한 변환
transform = T.Compose([
    T.Resize((640, 640)),  # 이미지 크기를 800으로 조정
    T.ToTensor(),  # 이미지를 Tensor로 변환
    T.Normalize(mean=[0.485, 0.456, 0.406],  # 이미지 정규화
                std=[0.229, 0.224, 0.225])
])

# 이미지를 로드하고 RGB로 변환
image = Image.open('/mnt/share/lvis/images/train2017/000000301121.jpg').convert('RGB')

# 리사이즈할 크기 지정
new_size = (640, 640)

image = resize_with_padding(image, new_size)

# # 이미지를 전처리
img = transform(image).unsqueeze(0).numpy()

# 원본 이미지의 크기를 저장
orig_size = np.array([[image.height, image.width]])
...
# 예측된 bounding box와 레이블을 이미지에
for box, label, score in zip(boxes_xyxy, label_names, scores):
    x0, y0, x1, y1 = box.astype(int)
    cv2.rectangle(image_cv, (x0, y0), (x1, y1), (0, 255, 0), 2)
    text = f'{label}: {score:.2f}'
    cv2.putText(image_cv, text, (x0, y0 - 5), cv2.FONT_HERSHEY_SIMPLEX,
                0.5, (0, 255, 0), 2)

# 결과 이미지를 저장
cv2.imwrite('EFL_test/RT-DETR-main/rtdetr_pytorch/output_image1.jpg', image_cv)
